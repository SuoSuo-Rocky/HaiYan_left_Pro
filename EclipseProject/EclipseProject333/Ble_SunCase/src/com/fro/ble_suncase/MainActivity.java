...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		Log.d("shi","----------onCreate-------------"+Thread.currentThread().getName());
	...
	private void bindView() {
		Log.d("shi","-----------bindView------------"+Thread.currentThread().getName());
	...
	private void initData() {
		Log.d("shi","-----------initData------------"+Thread.currentThread().getName());
		// 接收字段
	...
	private void initEvent() {
		Log.d("shi","-------- initEvent---------------"+Thread.currentThread().getName());
		...
		scan_tb.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				Log.d("shi","------------scan_tb.setOnCheckedChangeListener(-----------"+Thread.currentThread().getName());
				if (isChecked) {
					// 清空列表
		...
		bleDev_lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				Log.d("shi","--------- onItemClick--------------"+Thread.currentThread().getName());
				// 停止扫描
	...
	private BluetoothAdapter.LeScanCallback mLeScanCallback = new BluetoothAdapter.LeScanCallback() {
		...
		public void onLeScan(final BluetoothDevice device, int rssi, byte[] scanRecord) {
			Log.d("shi","------------.LeScanCallback-----------"+Thread.currentThread().getName());
	...
	private BluetoothLeClass.OnServiceDiscoverListener mOnServiceDiscover = new OnServiceDiscoverListener() {
		...
		public void onServiceDiscover(BluetoothGatt gatt) {
			Log.d("shi","---------@Override-onServiceDiscover-------------"+Thread.currentThread().getName());
			// 寻找到服务
	...
	private BluetoothLeClass.OnDataAvailableListener mOnDataAvailable = new OnDataAvailableListener() {
		...
		public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
			Log.d("shi","------@Override  onCharacteristicRead(-----------------"+Thread.currentThread().getName());
		...
		public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
			...
			if (characteristic.getUuid().toString().equals(UUID_CHAR6)) {
				Log.d("shi","--------@Override  onCharacteristicWrite---------------"+Thread.currentThread().getName());
				// 接收数据
	...
	private void ReadBle() {
		...
		if (mBLE.getmBluetoothGatt() != null && gattCharacteristic_char6 != null) {
			Log.d("shi","--------ReadBle---------------"+Thread.currentThread().getName());
			// 获取发送命令
